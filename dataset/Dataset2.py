...
class Dataset(torch.utils.data.Dataset):
    ...
    def augmentation(self,  ct_instance_layer_clipped_normalized_rotated_resized_cropped,
        ...
        if self.train_mode and self.params_dict.get("data.augmentation"):
            ...
            if brightness_contrast_sharpness_prob > brightness_contrast_sharpness_threshold:
                # B 0.9, C 1.3
        ...
        if limit_max_number_of_layers:
            ...
            if(ct_instance_layer_number > max_number_of_layers):      
                ...
                for ct_instance_layer_index in range(max_number_of_layers):
                    ...
                    ct_instance_layer_clipped_rotated = np.rot90(ct_instance_layer_clipped)
                    ...
                    ct_instance_layer_clipped_rotated_resized = cv2.resize(ct_instance_layer_clipped_rotated, dsize=(height, width), interpolation=cv2.INTER_CUBIC)

                    # Because max and min can be the same if the whole ct layer is empty
                    upper_bound = ct_instance_layer_clipped_rotated_resized.max()
                    lower_bound = ct_instance_layer_clipped_rotated_resized.min()
                    if not (upper_bound == lower_bound):
                        ct_instance_layer_clipped_rotated_resized_normalized = (ct_instance_layer_clipped_rotated_resized - (lower_bound)) / ((upper_bound) - (lower_bound))
                    else:
                        ct_instance_layer_clipped_rotated_resized_normalized = ct_instance_layer_clipped_rotated_resized
                    #plt.imshow(ct_instance_layer_clipped_rotated_resized_normalized, cmap='gray')
                    #plt.savefig("/home/guevenira/attention_CT/PDAC/debugging/test1235.png")
                    #plt.close()                    

                    ct_instance_layer_clipped_rotated_resized_normalized_cropped = ct_instance_layer_clipped_rotated_resized_normalized[crop_height_begin:crop_height_end, crop_width_begin:crop_width_end]
                    
                    ct_instance_layer_clipped_rotated_resized_normalized_cropped = self.augmentation(       ct_instance_layer_clipped_rotated_resized_normalized_cropped,
                                                                                                            horizontal_flip_prob, horizontal_flip_threshold, 
                                                                                                            translation_prob, translation_threshold, M, 
                                                                                                            gaussian_prob, gaussian_threshold, noise, amin, amax, 
                                                                                                            brightness_contrast_sharpness_prob, brightness_contrast_sharpness_threshold)
                    ...
                    if ct_instance_tensor == []:
                        ct_instance_tensor = torch.tensor(ct_instance_layer_clipped_rotated_resized_normalized_cropped.copy(), dtype=torch.float)
                        ct_instance_tensor = torch.unsqueeze(ct_instance_tensor, 0)
                    else:
                        ct_instance_tensor_new_layer = torch.tensor(ct_instance_layer_clipped_rotated_resized_normalized_cropped.copy(), dtype=torch.float)
