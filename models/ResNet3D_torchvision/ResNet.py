...
from .NonLocal import NLBlockND
...
class VideoResNet(nn.Module):
    ...
    def __init__(
        ...
        self.layer1 = self._make_layer(block, conv_makers[0], 64, layers[0], stride=1, non_local=False)
        self.layer2 = self._make_layer(block, conv_makers[1], 128, layers[1], stride=2, non_local=False)
        self.layer3 = self._make_layer(block, conv_makers[2], 256, layers[2], stride=2, non_local=True)
        self.layer4 = self._make_layer(block, conv_makers[3], 512, layers[3], stride=2, non_local=False)
    ...
    def _make_layer(
        ...
        # modify mode and dimension params
        # NORMALLY 1024, but change that!!!!
        print(non_local)
        if non_local:
