...
class ResNet50_maxpool_MultiheadAttention(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        feature_extractor = models.resnet.resnet50(weights=models.ResNet50_Weights.DEFAULT)
        ...
        self.kernel_att_after_layer1 = nn.MultiheadAttention(3136, 56)
    ...
    def forward(self, x):
        ...
        x = self.feature_extractor.layer1(x)    #print("layer1", x.shape)   - [110, 256, 56, 56]
        # Kernal attention pooling between layer 1 and layer 2
        """"""
        layer_avg = x.mean(0, keepdims=True)                    # [1, 256, 56, 56]
        
        kernel_features = torch.flatten(layer_avg, 2)           # [1, 256, 3136]
        kernel_features = kernel_features.permute(1, 0, 2)      # [256, 1, 3136]
        kernel_query = kernel_features.mean(0, keepdims=True)   # [1, 1, 3136]

        kernel_features, kernel_att_map = self.kernel_att_after_layer1(kernel_query, kernel_features, kernel_features)
        
        kernel_att_map = kernel_att_map.permute(2, 0 ,1)        # [256, 1, 1]
        kernel_att_map = kernel_att_map.unsqueeze(0)            # [1, 256, 1, 1]

        layer_avg_att = layer_avg * kernel_att_map              # [1, 256, 56, 56]
        
        x = x * layer_avg_att                                   # [110, 256, 56, 56]
        """"""
